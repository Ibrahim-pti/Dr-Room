...
<form id="profile-form" action="{{ route('lab.profile.update') }}" method="POST" enctype="multipart/form-data" class="bg-white rounded-2xl shadow-sm border border-slate-200/60 p-6 max-w-4xl">
    ...
    <div class="space-y-6">
            ...
            <div class="space-y-4">
                <div>
                    ...
                    <input type="text" id="name" name="name" value="{{ old('name', $user->name) }}" required
                        class="w-full px-4 py-2.5 bg-slate-50 border-slate-200 rounded-xl focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all font-medium text-slate-700">
                ...
                <div>
                    ...
                    <input type="text" id="name_en" name="name_en" value="{{ old('name_en', $user->name_en) }}" dir="ltr"
                        class="w-full text-left px-4 py-2.5 bg-slate-50 border-slate-200 rounded-xl focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all font-medium text-slate-700">
                </div>
            </div>
            ...
            <div class="space-y-4">
                <div>
                    ...
                    <input type="text" id="phone" name="phone" value="{{ old('phone', $user->phone) }}" required dir="ltr"
                        class="w-full text-right px-4 py-2.5 bg-slate-50 border-slate-200 rounded-xl focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all font-medium text-slate-700">
                    @error('phone') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
                </div>
                <div>
                    <label for="license_number" class="block text-sm font-medium text-slate-700 mb-2">ژمارەی مۆڵەت</label>
                    <input type="text" id="license_number" name="license_number" value="{{ old('license_number', $lab?->license_number) }}" dir="ltr"
                        class="w-full text-right px-4 py-2.5 bg-slate-50 border-slate-200 rounded-xl focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all font-medium text-slate-700">
                </div>
                <div>
                    <label for="equipment_level" class="block text-sm font-medium text-slate-700 mb-2">ئاستی ئامێرەکان</label>
                    <select id="equipment_level" name="equipment_level" class="w-full px-4 py-2.5 bg-slate-50 border-slate-200 rounded-xl focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all font-medium text-slate-700">
                        <option value="">هەڵبژێرە...</option>
                        <option value="basic" {{ old('equipment_level', $lab?->equipment_level) == 'basic' ? 'selected' : '' }}>سەرەتایی (Basic)</option>
                        <option value="standard" {{ old('equipment_level', $lab?->equipment_level) == 'standard' ? 'selected' : '' }}>مامناوەند (Standard)</option>
                        <option value="advanced" {{ old('equipment_level', $lab?->equipment_level) == 'advanced' ? 'selected' : '' }}>پێشکەوتوو (Advanced)</option>
                    </select>
                </div>
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 pt-4 border-t border-slate-100">
            ...
            <div class="space-y-4">
                <div>
                    <label for="location" class="block text-sm font-medium text-slate-700 mb-2">ناونیشان (کوردی) <span class="text-red-500">*</span></label>
                    <textarea id="location" name="location" rows="2" required
                        class="w-full px-4 py-2.5 bg-slate-50 border-slate-200 rounded-xl focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all font-medium text-slate-700">{{ old('location', $lab?->location) }}</textarea>
                </div>
                <div>
                    <label for="location_ar" class="block text-sm font-medium text-slate-700 mb-2">ناونیشان (عەرەبی)</label>
                    <textarea id="location_ar" name="location_ar" rows="2" dir="rtl"
                        class="w-full px-4 py-2.5 bg-slate-50 border-slate-200 rounded-xl focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all font-medium text-slate-700">{{ old('location_ar', $lab?->location_ar) }}</textarea>
                </div>
                <div>
                    <label for="location_en" class="block text-sm font-medium text-slate-700 mb-2">ناونیشان (ئینگلیزی)</label>
                    <textarea id="location_en" name="location_en" rows="2" dir="ltr"
                        class="w-full text-left px-4 py-2.5 bg-slate-50 border-slate-200 rounded-xl focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all font-medium text-slate-700">{{ old('location_en', $lab?->location_en) }}</textarea>
                </div>
            </div>
            ...
            <div class="space-y-4">
                <div>
                    ...
                    <textarea id="about_us" name="about_us" rows="2" required
                        class="w-full px-4 py-2.5 bg-slate-50 border-slate-200 rounded-xl focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all font-medium text-slate-700">{{ old('about_us', $lab?->about_us) }}</textarea>
                </div>
                <div>
                    ...
                    <textarea id="about_us_ar" name="about_us_ar" rows="2" dir="rtl"
                        class="w-full px-4 py-2.5 bg-slate-50 border-slate-200 rounded-xl focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all font-medium text-slate-700">{{ old('about_us_ar', $lab?->about_us_ar) }}</textarea>
                </div>
                <div>
                    ...
                    <textarea id="about_us_en" name="about_us_en" rows="2" dir="ltr"
                        class="w-full text-left px-4 py-2.5 bg-slate-50 border-slate-200 rounded-xl focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all font-medium text-slate-700">{{ old('about_us_en', $lab?->about_us_en) }}</textarea>
                </div>
            </div>
        </div>
        ...
        <div class="pt-6 border-t border-slate-100 flex items-center gap-4">
            <button type="submit" id="submitBtn" class="px-8 py-3 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl font-bold transition-colors shadow-lg shadow-indigo-200">
                پاشەکەوتکردن
            </button>
        </div>
    </div>
            ...
            if (sourceEl && sourceEl.value.trim()) {
                if (arEl && !arEl.value.trim()) {
                    const res = await fetch('/api/translate', {
                        ...
                        body: JSON.stringify({ text: sourceEl.value, target: 'ar' })
                    });
                    const data = await res.json();
                    if (data.translated_text) arEl.value = data.translated_text;
                }
                ...
                if (enEl && !enEl.value.trim()) {
                    const res = await fetch('/api/translate', {
                        ...
                        body: JSON.stringify({ text: sourceEl.value, target: 'en' })
                    });
                    const data = await res.json();
                    if (data.translated_text) enEl.value = data.translated_text;
                }
            }
        }
    } catch (e) {
