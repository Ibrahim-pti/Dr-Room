...
<div class="space-y-8 animate-fade-in-up">
        ...
        <div class="relative z-10 flex flex-col md:flex-row items-center justify-between">
            <div class="mb-6 md:mb-0">
                <span class="inline-block py-1 px-3 rounded-full bg-white/20 backdrop-blur-md border border-white/30 text-xs font-semibold tracking-wider uppercase mb-4 shadow-sm">
                    {{ $nurse->specialty ?? 'پزیشکی گشتی' }}
                </span>
                <h2 class="text-3xl md:text-4xl font-bold mb-2 tracking-tight">بەخێربێیتەوە پەرستار {{ explode(' ', $user->name)[0] }}!</h2>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div class="bg-white rounded-2xl p-6 shadow-[0_4px_20px_rgba(0,0,0,0.03)] border border-slate-100 flex items-center group hover:-translate-y-1 transition-all duration-300">
            ...
            <div>
                <p class="text-slate-500 text-sm font-medium mb-1">سەردانەکانی ئەمڕۆ</p>
        ...
        <div class="bg-white rounded-2xl p-6 shadow-[0_4px_20px_rgba(0,0,0,0.03)] border border-slate-100 flex items-center group hover:-translate-y-1 transition-all duration-300">
            ...
            <div>
                ...
                <div class="flex items-center space-x-2 space-x-reverse">
                    <p class="text-3xl font-bold text-slate-800">{{ number_format($nurse->rating ?? 0, 1) }}</p>
                    <span class="text-sm text-slate-400">/ 5.0 ({{ $nurse->total_reviews ?? 0 }})</span>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="bg-white rounded-2xl shadow-[0_4px_24px_rgba(0,0,0,0.02)] border border-slate-100 overflow-hidden">
        <div class="px-8 py-6 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
            <div>
                <h3 class="text-lg font-bold text-slate-800">سەردانەکانی داهاتوو</h3>
                    ...
                    <div class="p-6 hover:bg-slate-50 transition-colors flex items-center justify-between">
                        <div class="flex items-center">
                            ...
                            <div>
                                ...
                                <p class="text-sm text-slate-500">{{ $appointment->appointment_date->format('Y-m-d h:i A') }} - {{ $appointment->type == 'online' ? 'ئۆنلاین' : 'لە خزمەتگوزاری' }}</p>
                            </div>
                        </div>
                        <div>
        ...
        @else
            <div class="p-8 text-center text-slate-500 py-16">
                ...
                <h4 class="text-lg font-medium text-slate-700 mb-1">هیچ سەردانێک نییە</h4>
                <p class="text-sm max-w-sm mx-auto">کاتێک نەخۆشەکان سەردانیان لەگەڵت تۆمارکرد، لێرە دەردەکەون.</p>
            </div>
        @endif
    </div>
</div>
